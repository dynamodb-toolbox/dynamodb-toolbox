...
module.exports = (DocumentClient) => (attributes,linked,item,include=[],omit=[]) => {
  ...
  return Object.keys(item).reduce((acc,field) => {
    ...
    if (linked[field]) {
      Object.assign(acc, linked[field].reduce((acc,f,i) => {
        if (attributes[f].save || attributes[f].hidden || omit.includes(f) || (include.length > 0 && !include.includes(f))) return acc
    ...
    if ((attributes[field] && attributes[field].hidden) || omit.includes(field) || (include.length > 0 && !include.includes(field))) return acc
    return Object.assign(acc,{
