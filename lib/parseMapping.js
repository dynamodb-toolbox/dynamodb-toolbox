        ...
        if (typeof config[prop] === 'boolean' || typeof config[prop] === 'string' || Array.isArray(config[prop])) {
          ...
          for (let i in indexes) {
            if (typeof indexes[i] === 'boolean') {
              ...
              if (track.keys.partitionKey === track.keys.sortKey) error(`'${field}' attribute cannot be both the partitionKey and sortKey`)
            } else {
          ...
          error(`'${prop}' must be a boolean or a string`) 
        }
        break
      default:
