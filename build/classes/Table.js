...
const conditonError = (op) => error(`You can only supply one sortKey condition per query. Already using '${op}'`);
// Declare Table class
class Table {
                    ...
                    return Object.assign(result, {
                        Items: result.Items && result.Items.map(item => {
                            if (this[item[this.Table.entityField]]) {
                                return this[item[this.Table.entityField]].parse(item, 
            ...
            if (options.execute || (this.autoExecute && options.execute !== false)) {
                ...
                if (options.parse || (this.autoParse && options.parse !== false)) {
                    return Object.assign(result, {
                        Items: result.Items && result.Items.map(item => {
                            if (this[item[this.Table.entityField]]) {
                                return this[item[this.Table.entityField]].parse(item, EntityProjections[item[this.Table.entityField]] ? EntityProjections[item[this.Table.entityField]]
                                    : TableProjections ? TableProjections
                                        : []);
                            }
                            else {
                ...
                return Object.assign(acc, {
                    ...
                    [(Tables[table] && Tables[table].alias) || table]: result.Responses[table].map(item => {
                        ...
                        if (Tables[table] && Tables[table][item[Tables[table].Table.entityField]]) {
                            ...
                            return Tables[table][item[Tables[table].Table.entityField]].parse(item, EntityProjections[table] && EntityProjections[table][item[Tables[table].Table.entityField]] ? EntityProjections[table][item[Tables[table].Table.entityField]]
    ...
    parse(entity, input, include = []) {
        return __awaiter(this, void 0, void 0, function* () {
            if (!this[entity])
                error(`'${entity}' is not a valid Entity`);
