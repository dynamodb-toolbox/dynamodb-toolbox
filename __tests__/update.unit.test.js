...
describe('update',()=>{
  ...
  it('creates default update', () => {
    let { TableName, Key, UpdateExpression, ExpressionAttributeNames, ExpressionAttributeValues } = TestEntity.updateSync({ pk: 'test-pk', sk: 'test-sk' })
  ...
  it('creates update with multiple fields (default types)', () => {
    let { TableName, Key, UpdateExpression, ExpressionAttributeNames, ExpressionAttributeValues } = TestEntity.updateSync({
      pk: 'test-pk',
      sk: 'test-sk',
  ...
  it('creates update that just removes a field', () => {
    let { TableName, Key, UpdateExpression, ExpressionAttributeNames, ExpressionAttributeValues } = TestEntity2.updateSync({
      pk: 'test-pk',
      test: null
    })
  ...
  it('fails with undefined input', () => {
    expect(() => TestEntity.updateSync()).toThrow(`'pk' or 'email' is required`)
  })

  it('fails when using an undefined schema field', () => {
    expect(() => TestEntity.updateSync({
      'pk': 'test-pk',
      'sk': 'test-sk',
      'unknown': '?'
  ...
  it('adds statements to SET, REMOVE, ADD and DELETE (with names and values) and a ConditionExpression', ()=> {
    let { TableName, Key, UpdateExpression, ExpressionAttributeNames, ExpressionAttributeValues, ConditionExpression } = TestEntity2.updateSync({
      'pk': 'test-pk',
      'test': 'test'
    }, {
      SET: ['#field = :field'],
  ...
  it('conditionally contains returned fields (e.g. when no values)', ()=> {
    let params = TestEntity2.updateSync({
      'pk': 'test-pk',
      'test': null
    })
