...
import type { ToDBTable } from './dbTable.js'
...
> {
  public tables: {
    [KEY in keyof TABLES]: ToDBTable<TABLES[KEY]>
  }

  constructor(tables = {} as TABLES) {
    this.tables = Object.fromEntries(
      Object.entries(tables).map(([key, table]) => [key, DBTable.fromTable(table)])
    ) as { [KEY in keyof TABLES]: ToDBTable<TABLES[KEY]> }
  }
