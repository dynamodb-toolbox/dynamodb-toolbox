...
// Import parseTable
import { ParsedTable, parseTable } from '../lib/parseTable'
...
class Table {
  ...
  constructor(table: DynamoToolbox.TableConstructor) {
    //},docClient,entities) {
  ...
  set DocumentClient(docClient: DocumentClient) {
    ...
    if (
      ...
      if ((<any>docClient).options.convertEmptyValues !== false)
        (<any>docClient).options.convertEmptyValues = true
  ...
  /**
   * @deprecated this setter was maintained for backward compatibility, please use .addEntity instead
   * @param entity
   */
  set entities(entity: Entity | Entity[]) {
    this.addEntity(entity)
  }

  /**
   * @deprecated this getter was maintained for backward compatibility, please use .addEntity instead
   */
  get entities() {
  ...
  async query(pk, options: DynamoToolbox.QueryInputOptions = {}, params = {}) {
          ...
          result,
          {
            Items:
              ...
              result.Items.map((item) => {
                const index: DynamoToolbox.FIXME = this.Table.entityField
                if (this[item[index]]) {
                  return this[item[index]].parse(
                    item,
                    // Array.isArray(options.omit) ? options.omit : [],
                    EntityProjections[item[index]]
                      ? EntityProjections[item[index]]
                      : TableProjections
                      ? TableProjections
                      : []
  ...
  // SCAN the table
  async scan(options: DynamoToolbox.ScanInputOptions, params = {}) {
  ...
  async batchGet(
    items,
    options: DynamoToolbox.BachGetOptions = {},
    params = {}
  ) {
  ...
  parseBatchGetResponse(
    ...
    return Object.assign(
      ...
      result.UnprocessedKeys && Object.keys(result.UnprocessedKeys).length > 0
        ? {
            next: () => {
              const nextResult = this.DocumentClient.batchGet(
                Object.assign(
  ...
  batchGetParams(
    ...
    return meta
      ? { payload, Tables, EntityProjections, TableProjections }
      : payload
  } // batchGetParams

  // BatchGet Items
  async batchWrite(items, options: FIXME = {}, params = {}) {
    // Generate the payload with meta information
  ...
  parseBatchWriteResponse(result, options: FIXME = {}) {
    return Object.assign(
      result,
      // If UnprocessedKeys, return a next function
      result.UnprocessedKeys && Object.keys(result.UnprocessedKeys).length > 0
        ? {
            next: () => {
              const nextResult = this.DocumentClient.batchWrite(
                Object.assign(
                  { RequestItems: result.UnprocessedKeys },
                  options.capacity
                    ? { ReturnConsumedCapacity: options.capacity.toUpperCase() }
