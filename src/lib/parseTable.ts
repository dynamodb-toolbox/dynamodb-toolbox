...
// Parse table
export const parseTable = (table: DynamoToolbox.TableConstructor) => {
  let {
    name, // Table name
  ...
  if (Object.keys(args).length > 0)
    error(
      `Invalid Table configuration options: ${Object.keys(args).join(', ')}`
    )
...
const parseIndexes = (indexes, pk) =>
  Object.keys(indexes).reduce((acc, index) => {
    ...
    if (!sortKey && !partitionKey)
      error(
        `A 'partitionKey', 'sortKey' or both, must be provided for ${index}`
      )
