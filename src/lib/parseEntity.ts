...
export function parseEntity(entity: EntityConstructor) {
  ...
  if (Object.keys(args).length > 0)
    error(
      `Invalid Entity configuration options: ${Object.keys(args).join(", ")}`
    );
  ...
  modified =
    typeof modified === "string" && modified.trim().length > 0
      ? modified.trim()
      : "_md";
  ...
  // Define 'entityAlias'
  typeAlias =
    typeof typeAlias === "string" && typeAlias.trim().length > 0
      ? typeAlias.trim()
      : "entity";
