...
export function* tupleSchemaParser<OPTIONS extends ParseAttrValueOptions = {}>(
...
): Generator<ParserYield<TupleSchema, OPTIONS>, ParserReturn<TupleSchema, OPTIONS>> {
  ...
  if (isInputValueArray) {
    parsers = schema.elements.map((element, index) =>
      schemaParser(element, inputValue[index], {
        ...restOptions,
  ...
  if (fill) {
    if (isInputValueArray) {
      ...
      yield defaultedValue as ParserYield<TupleSchema, OPTIONS>

      const linkedValue = defaultedValue
      yield linkedValue as ParserYield<TupleSchema, OPTIONS>
    }
  }
