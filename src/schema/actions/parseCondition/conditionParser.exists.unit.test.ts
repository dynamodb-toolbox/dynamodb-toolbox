...
describe('parseCondition - exists', () => {
  const simpleSchema = item({
    num: number()
  })

  test('exists', () => {
    expect(simpleSchema.build(ConditionParser).parse({ attr: 'num', exists: true })).toStrictEqual({
  ...
  test('not exists', () => {
    expect(simpleSchema.build(ConditionParser).parse({ attr: 'num', exists: false })).toStrictEqual(
      {
  ...
  test('deep maps', () => {
    expect(
      mapSchema.build(ConditionParser).parse({ attr: 'map.deepA.deepB', exists: true })
    ).toStrictEqual({
  ...
  test('deep lists', () => {
    expect(
      listSchema.build(ConditionParser).parse({ attr: 'list[1][2][3]', exists: true })
    ).toStrictEqual({
