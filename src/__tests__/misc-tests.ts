...
beforeAll(async () => {
  ...
  await DynamoDB.createTable(
    Object.assign(createTableParams, { TableName: 'test-table' })
  ).promise()
})
...
describe('Table creation', () => {
  it('misc tests', async () => {
    ...
    await Default.User.put({
      ...
      age: 50,
      roles: ['housekeeper'],
      mapTest: { test: 'Test level 1', test2: { test3: 'Test level 2' } },
    })
    await Default.User.put({
      family: 'Brady',
      name: 'Greg',
    ...
    console.log(
      JSON.stringify(await Default.query('Brady', { eq: 'Mike' }), null, 2)
    )
