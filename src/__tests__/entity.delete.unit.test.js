...
describe('delete', () => {
    ...
    it('allows execute and parse options', () => {
        const { TableName, Key } = TestEntity.deleteParams({ email: 'x', sort: 'y' }, { execute: false, parse: false });
        expect(TableName).toBe('test-table');
    ...
    it('sets capacity options', () => {
        const { TableName, Key, ReturnConsumedCapacity } = TestEntity.deleteParams({ email: 'x', sort: 'y' }, { capacity: 'none' });
        expect(TableName).toBe('test-table');
        expect(Key).toEqual({ pk: 'x', sk: 'y' });
        expect(ReturnConsumedCapacity).toBe('NONE');
    });
    it('sets metrics options', () => {
        const { TableName, Key, ReturnItemCollectionMetrics } = TestEntity.deleteParams({ email: 'x', sort: 'y' }, { metrics: 'size' });
        expect(TableName).toBe('test-table');
        expect(Key).toEqual({ pk: 'x', sk: 'y' });
        expect(ReturnItemCollectionMetrics).toBe('SIZE');
    });
    it('sets returnValues options', () => {
        const { TableName, Key, ReturnValues } = TestEntity.deleteParams({ email: 'x', sort: 'y' }, { returnValues: 'ALL_OLD' });
        expect(TableName).toBe('test-table');
        expect(Key).toEqual({ pk: 'x', sk: 'y' });
        expect(ReturnValues).toBe('ALL_OLD');
    });
    it('sets conditions', () => {
        const result = TestEntity.deleteParams({ email: 'x', sort: 'y' }, { conditions: { attr: 'email', gt: 'test' } });
    ...
    it('handles extra parameters', () => {
        const { TableName, Key, ReturnConsumedCapacity } = TestEntity.deleteParams({ email: 'x', sort: 'y' }, {}, { ReturnConsumedCapacity: 'NONE' });
        expect(TableName).toBe('test-table');
        expect(Key).toEqual({ pk: 'x', sk: 'y' });
        expect(ReturnConsumedCapacity).toBe('NONE');
    });
    it('handles invalid parameter input', () => {
        const { TableName, Key } = TestEntity.deleteParams({ email: 'x', sort: 'y' }, {}, 
        // @ts-expect-error
        'string');
        expect(TableName).toBe('test-table');
        expect(Key).toEqual({ pk: 'x', sk: 'y' });
