...
describe('Entity creation', () => {
    ...
    it('fails when attribute has an invalid type (string style)', () => {
        ...
        expect(result).toThrow(`Invalid or missing type for 'pk'. Valid types are 'string', 'boolean', 'number', 'bigint', 'list', 'map', 'binary', and 'set'.`);
    });
    ...
    it('requires wrapNumbers if DocumentClient provided', () => {
        ...
        expect(() => new Entity({ ...entityDefn, table: TestTable1 })).toThrow('Please set `wrapNumbers: true` in your DocumentClient to avoid losing precision with bigint fields');
