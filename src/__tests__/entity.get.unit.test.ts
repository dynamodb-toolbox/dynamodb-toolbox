...
const TestEntityWithRequiredAndMappedProperties = new Entity({
  ...
  attributes: {
    ...
    statusNumber: {
      ...
      onUpdate: true,
      dependsOn: 'status',
      default: ({ status }: any) => status.length
    }
  },
  table: TestTable
} as const)
...
describe('get', () => {
  test('gets the key from inputs (sync)', async () => {
  ...
  test('should successfully run when missing non partition key properties', async () => {
    const { TableName, Key } = TestEntityWithRequiredAndMappedProperties.getParams({
      pk: 'test-pk',
      sk: 'test-sk'
    })
    expect(TableName).toBe('test-table')
  ...
  test('sets consistent and capacity options', () => {
