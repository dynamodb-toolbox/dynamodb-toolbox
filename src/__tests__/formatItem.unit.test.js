...
describe('formatItem', () => {
    ...
    it('specifies attributes to include', () => {
        const result = formatItem()(DefaultTable.User.schema.attributes, DefaultTable.User.linked, { pk: 'test' }, ['pk']);
        expect(result).toEqual({ pk: 'test' });
    });
    it('specifies attributes to include', () => {
        const result = formatItem()(DefaultTable.User.schema.attributes, DefaultTable.User.linked, { list: ['test'] }, ['list']);
        expect(result).toEqual({ list_alias: ['test'] });
    });
    it('specifies attributes to include with linked fields', () => {
        const result = formatItem()(DefaultTable.User.schema.attributes, DefaultTable.User.linked, { sk: 'test1#test2' }, ['linked1']);
