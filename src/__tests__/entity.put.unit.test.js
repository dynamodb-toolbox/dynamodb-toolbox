...
describe('put', () => {
    ...
    it('omits unmapped attributes when strictSchemaCheck is false.', () => {
        const { Item } = TestEntity.putParams({ email: 'x', sort: 'y', unknown: '?' }, { strictSchemaCheck: false });
        assert.ok(Item !== undefined, 'Item is undefined');
    ...
    it('formats a batch put response', async () => {
        ...
        assert.ok(result['test-table'].PutRequest !== undefined, 'result[\'test-table\'].PutRequest is undefined');
    ...
    it('sets capacity options', () => {
        const { TableName, ReturnConsumedCapacity } = TestEntity.putParams({ email: 'x', sort: 'y' }, { capacity: 'none' });
        expect(TableName).toBe('test-table');
        expect(ReturnConsumedCapacity).toBe('NONE');
    });
    it('sets metrics options', () => {
        const { TableName, ReturnItemCollectionMetrics } = TestEntity.putParams({ email: 'x', sort: 'y' }, { metrics: 'size' });
        expect(TableName).toBe('test-table');
        expect(ReturnItemCollectionMetrics).toBe('SIZE');
    });
    it('sets returnValues options', () => {
        const { TableName, ReturnValues } = TestEntity.putParams({ email: 'x', sort: 'y' }, { returnValues: 'ALL_OLD' });
        expect(TableName).toBe('test-table');
    ...
    it('sets conditions', () => {
        const { TableName, ExpressionAttributeNames, ExpressionAttributeValues, ConditionExpression } = TestEntity.putParams({ email: 'x', sort: 'y' }, { conditions: { attr: 'email', gt: 'test' } });
        expect(TableName).toBe('test-table');
    ...
    it('handles extra parameters', () => {
        const { TableName, ReturnConsumedCapacity } = TestEntity.putParams({ email: 'x', sort: 'y' }, {}, { ReturnConsumedCapacity: 'NONE' });
        expect(TableName).toBe('test-table');
        expect(ReturnConsumedCapacity).toBe('NONE');
    });
    it('handles invalid parameter input', () => {
        const { TableName } = TestEntity.putParams({ email: 'x', sort: 'y' }, {}, 
        // @ts-expect-error
        'string');
        expect(TableName).toBe('test-table');
    });
