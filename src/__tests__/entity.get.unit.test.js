...
describe('get', () => {
    ...
    it('allows execute and parse options', () => {
        const { TableName, Key } = TestEntity.getParams({ email: 'x', sort: 'y' }, { execute: false, parse: false });
        expect(TableName).toBe('test-table');
    ...
    it('parses attribute projections', () => {
        const { TableName, Key, ExpressionAttributeNames, ProjectionExpression } = TestEntity.getParams({ email: 'x', sort: 'y' }, { attributes: ['email'] });
        expect(TableName).toBe('test-table');
        expect(Key).toEqual({ pk: 'x', sk: 'y' });
    ...
    it('sets consistent and capacity options', () => {
        const { TableName, Key, ConsistentRead, ReturnConsumedCapacity } = TestEntity.getParams({ email: 'x', sort: 'y' }, { consistent: true, capacity: 'none' });
        expect(TableName).toBe('test-table');
        expect(Key).toEqual({ pk: 'x', sk: 'y' });
    ...
    it('handles extra parameters', () => {
        const { TableName, Key, ConsistentRead } = TestEntity.getParams({ email: 'x', sort: 'y' }, {}, { ConsistentRead: true });
        expect(TableName).toBe('test-table');
        expect(Key).toEqual({ pk: 'x', sk: 'y' });
        expect(ConsistentRead).toBe(true);
    });
    it('handles invalid parameter input', () => {
        const { TableName, Key } = TestEntity.getParams({ email: 'x', sort: 'y' }, {}, 
        // @ts-expect-error
        'string');
        expect(TableName).toBe('test-table');
        expect(Key).toEqual({ pk: 'x', sk: 'y' });
    ...
    it('formats a batch get response', async () => {
        ...
        expect(Table === null || Table === void 0 ? void 0 : Table.name).toBe('test-table');
