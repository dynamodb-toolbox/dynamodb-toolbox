...
describe('validateTypes', () => {
  ...
  test('fails with invalid string', async () => {
    expect(() => {
  ...
  test('fails with invalid number (coerce, "123abc")', async () => {
    expect(() => {
  ...
  // Number("") === 0
  test('fails with invalid number (coerce, "")', async () => {
    expect(() => {
  ...
  test('fails with invalid number (coerce, boolean)', async () => {
    expect(() => {
  ...
  // Number([123]) === 123
  test('fails with invalid number (coerce, [123])', async () => {
    expect(() => {
  ...
  test('fails with invalid number (coerce, "Infinity")', async () => {
    expect(() => {
  ...
  test('fails with invalid map', async () => {
    expect(() => {
  ...
  test('validates set', async () => {
    const result = validateTypes()({ type: 'set', setType: 'number' }, 'attr', new Set([1, 2, 3]))
    expect(result).toEqual(new Set([1, 2, 3]))
  })

  test('fails with invalid set', async () => {
    expect(() => {
      validateTypes()({ type: 'set', setType: 'string' }, 'attr', false)
    }).toThrow(`'attr' must be a valid set (array)`)
  })

  test('coerces strings to bigints', async () => {
    const result = validateTypes()(
      { type: 'bigint', coerce: true },
      'attr',
  ...
  test('converts arrays of bigints to sets', async () => {
    ...
    expect(result).toEqual(
      new Set([toDynamoBigInt(BigInt(-1234)), toDynamoBigInt(BigInt('123000000000000000000001'))])
    )
