...
describe('normalizeData', () => {
    ...
    it('filter out non-mapped fields', async () => {
        const result = normalizeData()(attributes, linked, { pk: 'test', $remove: 'testx', notAField: 'test123' }, true);
        expect(result).toEqual({
            pk: 'test'
        });
    });
        ...
        const ent = new Entity({
            ...
            attributes: {
                ...
                sk: { type: 'string', sortKey: true, dependsOn: ['parent'],
                    default: (data) => `parent#${data.parent}`,
                },
                parent: { type: 'string' },
            },
            table: DefaultTable
        });
        expect(() => ent.updateParams({})).toThrow(`Required field 'sk' depends on attribute(s), one or more of which can't be resolved (parent)`);
    });
});
