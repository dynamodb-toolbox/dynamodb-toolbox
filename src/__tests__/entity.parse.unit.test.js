...
describe('parse', () => {
    ...
    it('parses single item and includes certain fields', () => {
        const item = TestEntity.parse({ pk: 'test@test.com', sk: 'email', test_string: 'test', _et: 'TestEntity' }, ['email', 'sk']);
        expect(item).toEqual({
            email: 'test@test.com',
            test_type: 'email'
        });
    ...
    it('parses wrapped numbers', () => {
        const wrap = (value) => unmarshall({ valueToUnmarshall: { N: value.toString() } }, { wrapNumbers: true })
