...
import { DeleteBatchItemRequest } from './operation'
...
describe('deleteBatchItem', () => {
  it('returns the result in the correct format', async () => {
    const { Key } = TestEntity.build(DeleteBatchItemRequest)
  ...
  it('fails if no key is provided', () => {
    const invalidCall = () => TestEntity.build(DeleteBatchItemRequest).params()
