...
This action is mostly a **wrapper around the schema [`Formatter`](../../../4-schemas/18-actions/2-format.md) action**, so we highly recommend you read its dedicated documentation first.
...
| `attributes` | `Path<Entity>[]` |    -    | To specify a list of attributes to format (other attributes are omitted).<br/><br/>See the [`PathParser`](../20-parse-paths/index.md#paths) action for more details on how to write attribute paths. |
